...
`MainForm.cs` contains the user-authored main application window.

`MainForm.Designer.cs` is generated by the ModernFormsNext designer.

`MainForm.mfdesign` is the companion designer document. The Visual Studio extension opens it
through **View ModernFormsNext Designer** from `MainForm.cs`.
...
The project file marks `MainForm.cs` with `ModernFormsNextDesigner=true` and does not use the
classic Windows Forms `SubType=Form` marker.
