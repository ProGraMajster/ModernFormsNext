...
The commands below target the coordinated 1.11.0 release. They become publicly available after the
...
Install `ModernFormsNextDesigner.vsix` version `1.11.0` from the matching 1.11.0 release assets when
...
fix. Do not install an older VSIX when validating the 1.11.0 designer workflow. During
...
ModernFormsNext 1.11.0 is **Experimental**, uses an explicit activity/shared-surface host, and is not
