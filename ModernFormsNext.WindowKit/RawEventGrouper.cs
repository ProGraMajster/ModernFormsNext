...
internal class RawEventGrouper : IDisposable
{
    ...
    private void Dispatch(RawInputEventArgs ev)
    {
        ...
        //if (ev is RawPointerEventArgs { IntermediatePoints.Value: List<RawPointerPoint> list }) 
        //    list.Dispose();
    }
    ...
    private static void MergeEvents(RawPointerEventArgs last, RawPointerEventArgs current)
    {
        ...
        last.IntermediatePoints ??= new Lazy<IReadOnlyList<RawPointerPoint>?>(s_getListDelegate);
        ((List<RawPointerPoint>)last.IntermediatePoints.Value!).Add(new RawPointerPoint { Position = last.Position });
