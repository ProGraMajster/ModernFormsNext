...
internal partial class DefaultLayout : LayoutEngine
{
    ...
    private static void LayoutAutoSizedControls (IArrangedElement container)
    {
        ...
        for (var i = children.Count () - 1; i >= 0; i--) {
            ...
            if (CommonProperties.xGetAutoSizedAndAnchored (element)) {
                ...
                var proposedConstraints = LayoutUtils.s_maxSize;
                ...
                var prefSize = element.GetPreferredSize (proposedConstraints);
                ...
                if (CommonProperties.GetAutoSizeMode (element) == AutoSizeMode.GrowAndShrink) {
                    ...
                    if (bounds.Width < prefSize.Width || bounds.Height < prefSize.Height) {
                        var newSize = LayoutUtils.UnionSizes (bounds.Size, prefSize);
    ...
    {
        var direction = GetGrowthDirection (element);
        var oldBounds = GetCachedBounds (element);
        var location = oldBounds.Location;
        ...
        var newBounds = new Rectangle (location, newSize);
    ...
    {
        var anchor = GetAnchor (element);
        var growthDirection = GrowthDirection.None;
    ...
    private static Rectangle GetAnchorDestination (IArrangedElement element, Rectangle displayRect, bool measureOnly)
    {
        ...
        var layout = GetAnchorInfo (element);

        var left = layout.Left + displayRect.X;
        var top = layout.Top + displayRect.Y;
        var right = layout.Right + displayRect.X;
        var bottom = layout.Bottom + displayRect.Y;
        ...
        if (!measureOnly) {
            ...
            var cachedBounds = GetCachedBounds (element);
    ...
    private static Size LayoutDockedControls (IArrangedElement container, bool measureOnly)
    {
        ...
        var remainingBounds = measureOnly ? Rectangle.Empty : container.DisplayRectangle;
        ...
        for (var i = children.Count () - 1; i >= 0; i--) {
            ...
            if (CommonProperties.GetNeedsDockLayout (element)) {
                ...
                switch (GetDock (element)) {
                    case DockStyle.Top: {
                            var elementSize = GetVerticalDockedSize (element, remainingBounds.Size, measureOnly);
                    ...
                    case DockStyle.Bottom: {
                            var elementSize = GetVerticalDockedSize (element, remainingBounds.Size, measureOnly);
                            var newElementBounds = new Rectangle (remainingBounds.X, remainingBounds.Bottom - elementSize.Height, elementSize.Width, elementSize.Height);
                    ...
                    case DockStyle.Left: {
                            var elementSize = GetHorizontalDockedSize (element, remainingBounds.Size, measureOnly);
                            var newElementBounds = new Rectangle (remainingBounds.X, remainingBounds.Y, elementSize.Width, elementSize.Height);
                    ...
                    case DockStyle.Right: {
                            var elementSize = GetHorizontalDockedSize (element, remainingBounds.Size, measureOnly);
                            var newElementBounds = new Rectangle (remainingBounds.Right - elementSize.Width, remainingBounds.Y, elementSize.Width, elementSize.Height);
    ...
    private static void TryCalculatePreferredSizeDockedControl (IArrangedElement element, Rectangle newElementBounds, bool measureOnly, ref Size preferredSize, ref Rectangle remainingBounds)
    {
        if (measureOnly) {
            ...
            if (dockStyle != DockStyle.Fill) {
                ...
                var elementPrefSize = element.GetPreferredSize (neededSize);
    ...
    {
        var newSize = xGetDockedSize (element, remainingSize, /* constraints = */ new Size (remainingSize.Width, 1), measureOnly);
        if (!measureOnly) {
    ...
    {
        var newSize = xGetDockedSize (element, remainingSize, /* constraints = */ new Size (1, remainingSize.Height), measureOnly);
        if (!measureOnly) {
    ...
    {
        return TryCalculatePreferredSize (container, measureOnly: false, preferredSize: out var _);
    }
    ...
    private static bool TryCalculatePreferredSize (IArrangedElement container, bool measureOnly, out Size preferredSize)
    {
        ...
        var preferredSizeForDocking = Size.Empty;
        var preferredSizeForAnchoring = Size.Empty;
    ...
    private static void UpdateAnchorInfo (IArrangedElement element)
    {
        ...
        if (CommonProperties.GetNeedsAnchorLayout (element) && element.Container is not null) {
            var bounds = GetCachedBounds (element);
            var oldAnchorInfo = new AnchorInfo {
            ...
            var parentDisplayRect = element.Container.DisplayRectangle;
    ...
    public static void SetAnchor (IArrangedElement? container, IArrangedElement element, AnchorStyles value)
    {
        ...
        if (oldValue != value) {
            ...
            if (element.Container is not null) {
                var rightReleased = IsAnchored (oldValue, AnchorStyles.Right) && !IsAnchored (value, AnchorStyles.Right);
                var bottomReleased = IsAnchored (oldValue, AnchorStyles.Bottom) && !IsAnchored (value, AnchorStyles.Bottom);
    ...
    public static void SetDock (IArrangedElement element, DockStyle value)
    {
        ...
        if (GetDock (element) != value) {
            ...
            var dockNeedsLayout = CommonProperties.GetNeedsDockLayout (element);
    ...
    {
        if (element.Container is not null) {
            var dictionary = (IDictionary?)element.Container.Properties.GetObject (s_cachedBoundsProperty);
            if (dictionary is not null) {
                var bounds = dictionary[element];
    ...
    private static void ApplyCachedBounds (IArrangedElement container)
    {
        ...
        var dictionary = (IDictionary?)container.Properties.GetObject (s_cachedBoundsProperty);
        if (dictionary is not null) {
            foreach (DictionaryEntry entry in dictionary) {
                var element = (IArrangedElement)entry.Key;
    ...
    internal override Size GetPreferredSize (IArrangedElement container, Size proposedBounds)
    {
        ...
        TryCalculatePreferredSize (container, measureOnly: true, preferredSize: out var prefSize);
        return prefSize;
    }
    ...
    private static Size GetAnchorPreferredSize (IArrangedElement container)
    {
        ...
        var children = container.Children;
        for (var i = children.Count () - 1; i >= 0; i--) {
            IArrangedElement element = container.Children.ElementAt (i);
            if (!CommonProperties.GetNeedsDockLayout (element) && element.ParticipatesInLayout) {
                var anchor = GetAnchor (element);
                var margin = CommonProperties.GetMargin (element);
                var elementSpace = LayoutUtils.InflateRect (GetCachedBounds (element), margin);
                ...
                if (IsAnchored (anchor, AnchorStyles.Right)) {
                    ...
                    var anchorDest = GetAnchorDestination (element, Rectangle.Empty, /*measureOnly=*/true);
