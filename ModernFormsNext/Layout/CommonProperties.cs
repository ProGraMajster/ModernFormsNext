...
internal partial class CommonProperties
{
    ...
    internal static bool GetAutoSize (IArrangedElement element)
    {
        ...
        var value = state[_autoSizeSection];
        return value != 0;
    }
    ...
    {
        var padding = element.Properties.GetPadding (_marginProperty, out var found);
    ...
    {
        var size = element.Properties.GetSize (_maximumSizeProperty, out var found);
    ...
    {
        var size = element.Properties.GetSize (_minimumSizeProperty, out var found);
    ...
    {
        var rectangle = element.Properties.GetRectangle (_specifiedBoundsProperty, out var found);
    ...
    {
        var value = element.Properties.GetObject (_paddingProperty);
    ...
    {
        var originalBounds = CommonProperties.GetSpecifiedBounds (element);

        // PERF note: Bitwise operator usage intentional to optimize out branching.

        var xChangedButNotSpecified = ((specified & BoundsSpecified.X) == BoundsSpecified.None) & x != originalBounds.X;
        var yChangedButNotSpecified = ((specified & BoundsSpecified.Y) == BoundsSpecified.None) & y != originalBounds.Y;
        var wChangedButNotSpecified = ((specified & BoundsSpecified.Width) == BoundsSpecified.None) & width != originalBounds.Width;
        var hChangedButNotSpecified = ((specified & BoundsSpecified.Height) == BoundsSpecified.None) & height != originalBounds.Height;
    ...
    internal static void xClearAllPreferredSizeCaches (IArrangedElement start)
    {
        ...
        var controlsCollection = start.Children;
    ...
    {
        var size = element.Properties.GetSize (_preferredSizeCacheProperty, out var found);
    ...
    {
        var state = GetLayoutState (element);
        var result = state[_dockAndAnchorNeedsLayoutSection] != 0;
    ...
    {
        var state = GetLayoutState (element);
        var result = (state[_dockAndAnchorNeedsLayoutSection] != 0) && (state[_dockModeSection] == (int)DockAnchorMode.Anchor);
    ...
    {
        var state = GetLayoutState (element);
        var result = state[_dockModeSection] == (int)DockAnchorMode.Dock && element.ParticipatesInLayout;
    ...
    {
        var state = GetLayoutState (element);
        var value = state[_selfAutoSizingSection];
        return (value == 1);
    }
    ...
    {
        var state = GetLayoutState (element);
        var value = (AnchorStyles)state[_dockAndAnchorSection];
        var mode = (DockAnchorMode)state[_dockModeSection];
    ...
    internal static bool xGetAutoSizedAndAnchored (IArrangedElement element)
    {
        ...
        var result = (state[_autoSizeSection] != 0) && (state[_dockModeSection] == (int)DockAnchorMode.Anchor);
    ...
    {
        var state = GetLayoutState (element);
        var value = (DockStyle)state[_dockAndAnchorSection];
        var mode = (DockAnchorMode)state[_dockModeSection];
    ...
    {
        var state = GetLayoutState (element);
        var value = state[_flowBreakSection];
        return value == 1;
    }
    ...
    {
        var size = element.Properties.GetSize (_layoutBoundsProperty, out var found);
