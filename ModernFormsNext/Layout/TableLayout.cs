...
internal partial class TableLayout : LayoutEngine
{
    ...
    internal override void ProcessSuspendedLayoutEventArgs (IArrangedElement container, LayoutEventArgs args)
    {
        ...
        foreach (var propertyName in _propertiesWhichInvalidateCache) {
    ...
    private protected override bool LayoutCore (IArrangedElement container, LayoutEventArgs args)
    {
        ...
        var cellBorderWidth = containerInfo.CellBorderWidth;

        //shrink the size of the display rectangle so that we have space to draw the border
        var containerSize = container.DisplayRectangle.Size - new Size (cellBorderWidth, cellBorderWidth);
        ...
        var usedSpace = ApplyStyles (containerInfo, containerSize, /*measureOnly=*/false);
        ExpandLastElement (containerInfo, usedSpace, containerSize);
        var displayRect = (RectangleF)container.DisplayRectangle;
    ...
    internal override Size GetPreferredSize (IArrangedElement container, Size proposedConstraints)
    {
        ...
        var oldWidth = -1f;
        var prefSize = containerInfo.GetCachedPreferredSize (proposedConstraints, out var isCacheValid);
        ...
        var tempInfo = new ContainerInfo (containerInfo);
        var cellBorderWidth = containerInfo.CellBorderWidth;
        ...
        if (containerInfo.MaxColumns == 1 && containerInfo.ColumnStyles.Count > 0 && containerInfo.ColumnStyles[0].SizeType == SizeType.Absolute) {
            //shrink the size of the display rectangle so that we have space to draw the border
            var containerSize = container.DisplayRectangle.Size - new Size (cellBorderWidth * 2, cellBorderWidth * 2);
        ...
        var cellBorderSize = new Size (cellBorderWidth, cellBorderWidth);
    ...
    {
        var numCols = containerInfo.MaxColumns;
        var numRows = containerInfo.MaxRows;
        ...
        var minSpace = containerInfo.MinRowsAndColumns;
        var minColumn = containerInfo.MinColumns;
        var minRow = containerInfo.MinRows;

        var growStyle = containerInfo.GrowStyle;
        ...
        if (numCols > 0) {
            ...
            var estimatedCols = Math.Max ((int)Math.Ceiling ((float)minSpace / numRows), minColumn);
    ...
    private bool xAssignRowsAndColumns (ContainerInfo containerInfo, LayoutInfo[] childrenInfo, int maxColumns, int maxRows, TableLayoutPanelGrowStyle growStyle)
    {
        ...
        var numColumns = 0;
        var numRows = 0;
        var reservationGrid = new ReservationGrid ();

        var currentRow = 0;
        var currentCol = 0;

        var fixedElementIndex = -1;
        var flowElementIndex = -1;
        ...
        var fixedChildrenInfo = containerInfo.FixedChildrenInfo;

        //the element at the head of the absolutely positioned element queue
        var fixedElement = GetNextLayoutInfo (fixedChildrenInfo, ref fixedElementIndex, /*absolutelyPositioned*/true);

        //the element at the head of the non-absolutely positioned element queue
        var flowElement = GetNextLayoutInfo (childrenInfo, ref flowElementIndex, /*absolutelyPositioned*/false);

        while (fixedElement is not null || flowElement is not null) {
            var colStop = currentCol;
            int rowStop;
            ...
            if (flowElement is not null && (fixedElement is null || (!IsCursorPastInsertionPoint (fixedElement, flowElement.RowStart, colStop) && !IsOverlappingWithReservationGrid (fixedElement, reservationGrid, currentRow)))) {
                ...
                for (var j = 0; j < flowElement.RowStart - currentRow; j++)
    ...
    {
        for (var i = ++index; i < layoutInfo.Length; i++) {
    ...
    private bool IsOverlappingWithReservationGrid (LayoutInfo fixedLayoutInfo, ReservationGrid reservationGrid, int currentRow)
    {
        ...
        for (var rowOffset = fixedLayoutInfo.RowPosition - currentRow; rowOffset < fixedLayoutInfo.RowPosition - currentRow + fixedLayoutInfo.RowSpan; rowOffset++) {
            for (var colOffset = fixedLayoutInfo.ColumnPosition; colOffset < fixedLayoutInfo.ColumnPosition + fixedLayoutInfo.ColumnSpan; colOffset++) {
    ...
    {
        var prevRow = layoutInfo.RowStart;
        do {
    ...
    private Size ApplyStyles (ContainerInfo containerInfo, Size proposedConstraints, bool measureOnly)
    {
        ...
        foreach (var layoutInfo in containerInfo.ChildrenInfo) {
        ...
        var expandLastElementWidth = Math.Max (0, proposedConstraints.Width - preferredSize.Width);
    ...
    private void InitializeStrips (Strip[] strips, IList styles)
    {
        ...
        for (int i = 0; i < strips.Length; i++) {
            var style = i < styles.Count ? (TableLayoutStyle)styles[i]! : null;
            strip = strips[i];
    ...
    {
        var dontHonorConstraint = measureOnly;

        var sortedChildren = containerInfo.ChildrenInfo;
        ...
        if (dontHonorConstraint && (proposedConstraints.Width < short.MaxValue)) {
            if (containerInfo.Container is TableLayoutPanel tlp && tlp.Parent is not null && tlp.Parent.LayoutEngine == DefaultLayout.Instance) {
        ...
        foreach (var layoutInfo in sortedChildren) {
            var element = layoutInfo.Element;

            var columnSpan = layoutInfo.ColumnSpan;
            ...
            if (columnSpan > 1 || !IsAbsolutelySized (layoutInfo.ColumnStart, containerInfo.ColumnStyles)) {
                ...
                if ((columnSpan == 1 && layoutInfo.RowSpan == 1) &&
                    ...
                    var constrainingHeight = (int)containerInfo.RowStyles[layoutInfo.RowStart].Size;
                ...
                // tack on margins
                var margin = CommonProperties.GetMargin (element);
                minWidth += margin.Horizontal;
                maxWidth += margin.Horizontal;

                // distribute the minimum size, then maximum size over the columns
                var colStop = Math.Min (layoutInfo.ColumnStart + layoutInfo.ColumnSpan, containerInfo.Columns.Length);
        ...
        var width = DistributeStyles (containerInfo.CellBorderWidth, containerInfo.ColumnStyles, containerInfo.Columns, proposedConstraints.Width, dontHonorConstraint);
        ...
        if (dontHonorConstraint && width > proposedConstraints.Width && proposedConstraints.Width > 1) {
            ...
            var strips = containerInfo.Columns;
            var totalPercent = 0f;
            var totalPercentAllocatedSpace = 0;
            var styles = containerInfo.ColumnStyles;

            for (var i = 0; i < strips.Length; i++) {
                var strip = strips[i];
                if (i < styles.Count) {
                    var style = (TableLayoutStyle)styles[i];
            ...
            var currentOverflow = width - proposedConstraints.Width;

            var stealAmount = Math.Min (currentOverflow, totalPercentAllocatedSpace);

            for (var i = 0; i < strips.Length; i++) {
                if (i < styles.Count) {
                    var style = (TableLayoutStyle)styles[i];
                    if (style.SizeType == SizeType.Percent) {
                        var percentageOfTotal = style.Size / (float)totalPercent;
    ...
    private int InflateRows (ContainerInfo containerInfo, Size proposedConstraints, int expandLastElementWidth, bool measureOnly)
    {
        ...
        var multiplePercent = containerInfo.HasMultiplePercentColumns;
        ...
        foreach (var layoutInfo in sortedChildren) {
            ...
            if (rowSpan > 1 || !IsAbsolutelySized (layoutInfo.RowStart, containerInfo.RowStyles)) {
                var currentWidth = SumStrips (containerInfo.Columns, layoutInfo.ColumnStart, layoutInfo.ColumnSpan);
                ...
                var margin = CommonProperties.GetMargin (element);
                var minHeight = GetElementSize (element, new Size (currentWidth - margin.Horizontal, 0)).Height + margin.Vertical;
                var maxHeight = minHeight;
                var rowStop = Math.Min (layoutInfo.RowStart + layoutInfo.RowSpan, containerInfo.Rows.Length);
    ...
    internal int SumStrips (Strip[] strips, int start, int span)
    {
        ...
        for (var i = start; i < Math.Min (start + span, strips.Length); i++) {
            var strip = strips[i];
            size += strip.MinSize;
        }
    ...
    {
        var currentLength = 0;   //total length allocated so far
        var numUninitializedStrips = 0;  //number of strips whose Size is 0 and is not absolutely positioned
        ...
        var missingLength = desiredLength - currentLength;
            ...
            for (var i = stop - 1; i >= start; i--) {
                if (!IsAbsolutelySized (i, styles)) {
                    ...
                    if (!(i == strips.Length - 1)               //this is not the last strip
                        ...
                        var offset = Math.Min (sizeProxy.Size, missingLength);
        ...
        //there are some uninitialized strips
            ...
            var average = missingLength / numUninitializedStrips;
            //total number of uninitialized strips encountered so far
            var uninitializedStripIndex = 0;
            for (var i = start; i < stop; i++) {
                sizeProxy.Strip = strips[i];
    ...
    private int DistributeStyles (int cellBorderWidth, IList styles, Strip[] strips, int maxSize, bool dontHonorConstraint)
    {
        ...
        var totalPercent = 0f;
        var totalPercentAllocatedSpace = 0f;
        var totalAbsoluteAndAutoSizeAllocatedSpace = 0f;
        var hasAutoSizeColumn = false;
        ...
        var remainingSpace = maxSize - usedSpace;
        ...
        if (totalPercent > 0) {
            if (!dontHonorConstraint) {
                ...
                for (var i = 0; i < strips.Length; i++) {
                    var strip = strips[i];
                    var sizeType = i < styles.Count ? ((TableLayoutStyle)styles[i]!).SizeType : SizeType.AutoSize;
                    if (sizeType == SizeType.Percent) {
                        ...
                        var stripSize = (int)(style.Size * totalPercentAllocatedSpace / totalPercent);
                ...
                for (var i = 0; i < strips.Length; i++) {
                    ...
                    if (sizeType == SizeType.Percent) {
                        ...
                        var totalWidth = (int)Math.Round (((strip.MinSize * totalPercent) / style.Size));
        ...
        if (hasAutoSizeColumn && remainingSpace > 0) {
            for (var i = 0; i < strips.Length; i++) {
                ...
                if (sizeType == SizeType.AutoSize) {
                    var delta = Math.Min (strip.MaxSize - strip.MinSize, remainingSpace);
                    if (delta > 0) {
                        usedSpace += delta;
    ...
    private void SetElementBounds (ContainerInfo containerInfo, RectangleF displayRectF)
    {
        ...
        for (var i = 0; i < childrenInfo.Length; i++) {
            var layoutInfo = (LayoutInfo)childrenInfo[i];
            ...
            // do this.
            var colStop = currentCol + layoutInfo.ColumnSpan;
            var width = 0;
            ...
            var rowStop = currentRow + layoutInfo.RowSpan;
            var height = 0;
            for (var rowIndex = currentRow; rowIndex < rowStop && rowIndex < containerInfo.Rows.Length; rowIndex++) {
                height += containerInfo.Rows[rowIndex].MinSize;
            }

            var cellBounds = new Rectangle ((int)(startX + cellBorderWidth / 2.0f), (int)(top + cellBorderWidth / 2.0f), width - cellBorderWidth, height - cellBorderWidth);
            ...
            var elementMargin = CommonProperties.GetMargin (element);
            if (isContainerRTL) {
            ...
            var elementBounds = LayoutUtils.AlignAndStretch (GetElementSize (element, cellBounds.Size), cellBounds, anchorStyles);
    ...
    internal TableLayoutPanelCellPosition GetPositionFromControl (IArrangedElement container, IArrangedElement child)
    {
        ...
        var layoutInfo = GetLayoutInfo (child);
    ...
    {
        var layoutInfo = (LayoutInfo?)element.Properties.GetObject (_layoutInfoProperty);
    ...
    {
        var containerInfo = (ContainerInfo?)container.Properties.GetObject (_containerInfoProperty);
